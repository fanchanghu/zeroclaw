...
impl Agent {
    ...
    pub async fn turn(&mut self, user_message: &str) -> Result<String> {
        ...
        // transcript.
        let mut history_with_user = self.history.clone();
        history_with_user.push(ConversationMessage::Chat(user_msg.clone()));
        let active_dispatcher = {
            let base_provider_messages = self
                .tool_dispatcher
                .to_provider_messages(&history_with_user);
        ...
        // `rebuild_system_prompt_for_dispatcher` refreshed `self.history[0]`
        // with the active provider's tool-mode system prompt. Mirror that
        // update into `history_with_user` without re-cloning the entire history.
        history_with_user[0] = self.history[0].clone();

        let provider_messages = active_dispatcher.to_provider_messages(&history_with_user);
    ...
    pub async fn turn_streamed_with_steering_state(
        ...
        // The agent was built against the base provider, but a multimodal turn
        // may route to a vision provider with different native-tool support.
        // To make vision routing see this turn's user message (which may carry
        // `[IMAGE:...]` markers), resolve against a temporary transcript that
        // includes the user message, then rebuild the stored system prompt with
        // that provider's dispatcher.
        let mut history_with_user = self.history.clone();
        history_with_user.push(ConversationMessage::Chat(user_msg.clone()));
        let active_dispatcher = {
            let base_provider_messages = self
        ...
        if let (Some(cache), Some(key)) = (&self.response_cache, &cache_key) {
            if let Ok(Some(cached)) = cache.get(key) {
                ...
                // The cached turn is a single assistant response appended to
                // the seed user message. Replay the full [user, assistant] turn
                // into history and return it as the caller-facing new messages.
                let cache_hit_turn = vec![user_msg.clone(), ChatMessage::assistant(cached.clone())];
                self.history
                    .extend(Self::replay_loop_messages(&cache_hit_turn));
                self.trim_history();
                ...
                return Ok(StreamedTurnSuccess {
                    ...
                    new_messages: Self::replay_loop_messages(&cache_hit_turn),
                });
            }
            self.observer.record_event(&ObserverEvent::CacheMiss {
                cache_type: "response".into(),
        ...
        // been committed into `self.history`. It begins at 0 because the seed
        // user message (and any accepted steering messages in later rounds)
        // must be replayed exactly once when they are first appended.
            ...
            match loop_result {
                ...
                Err(error) => {
                    ...
                    if crate::agent::loop_::is_tool_loop_cancelled(&error) {
                        ...
                        // partial the loop persisted (replayed into history
                        // above, and into committed_response by the
                        // empty-committed rebuild). Provenance, not content
                        // sniffing: model-authored text can end with the marker
