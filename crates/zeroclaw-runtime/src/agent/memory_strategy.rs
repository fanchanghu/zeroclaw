...
impl MemoryStrategy for DefaultMemoryStrategy {
    ...
    async fn run_governance(&self) -> anyhow::Result<GovernanceReport> {
        ...
        // Phase 1: `hygiene::run_if_due` returns `Result<()>`; the structured
        // `GovernanceReport` will be wired in a follow-up when hygiene
    ...
    async fn feedback(&self, _entry_id: &str, _helpful: bool) -> anyhow::Result<()> {
        // Phase 1: no-op, reserved for follow-up.
        Ok(())
    }
}
